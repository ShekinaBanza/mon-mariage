...
const NAV: NavItem[] = [
  { href: "/admin", label: "Tableau de bord", icon: <LayoutDashboard className="h-4 w-4" />, permission: "dashboard:view" },
  { href: "/admin/guests", label: "Invites", icon: <Users className="h-4 w-4" />, permission: "guests:view" },
  { href: "/admin/tables", label: "Tables & chaises", icon: <Armchair className="h-4 w-4" />, permission: "tables:manage" },
  { href: "/admin/floorplan", label: "Plan de salle", icon: <Map className="h-4 w-4" />, permission: "floorplan:manage" },
  { href: "/admin/scan-logs", label: "Historique scans", icon: <ScanLine className="h-4 w-4" />, permission: "dashboard:view" },
  { href: "/admin/settings", label: "Parametres", icon: <Settings className="h-4 w-4" />, permission: "settings:manage" },
  { href: "/admin/lists", label: "Listes imprimables", icon: <Printer className="h-4 w-4" />, permission: "lists:print" },
];
...
export function AdminShell({
  ...
  const navItems = NAV.filter((item) => can(user.role, item.permission));
  const canUseScanner = can(user.role, "scanner:use");
  ...
  return (
    <div className="flex min-h-screen bg-ivory/30">
      <aside className={`admin-sidebar fixed inset-y-0 left-0 z-40 w-64 transform border-r border-gold/30 bg-card transition-transform lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between border-b border-gold/20 px-5 py-4">
            ...
            <button onClick={() => setOpen(false)} className="text-muted-foreground lg:hidden"><X className="h-5 w-5" /></button>
          </div>
          ...
          <nav className="flex-1 space-y-1 overflow-y-auto p-3 scrollbar-elegant">
            ...
            {(canUseScanner || navItems.length > 0) && <div className="my-2 border-t border-gold/15" />}
            {canUseScanner && (
              <Link href="/scan" target="_blank" className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-foreground/80 hover:bg-ivory hover:text-sage-deep">
                <ScanLine className="h-4 w-4" /> Scanner les entrees <ExternalLink className="ml-auto h-3 w-3" />
              </Link>
            )}
          ...
          <div className="border-t border-gold/20 p-3">
            <div className="rounded-lg bg-ivory/60 p-3">
              ...
              <Button onClick={logout} loading={loggingOut} loadingText="Deconnexion..." variant="ghost" size="sm" className="mt-2 w-full text-xs text-muted-foreground hover:text-destructive">
                <LogOut className="mr-2 h-3 w-3" /> Deconnexion
              </Button>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-1 flex-col lg:pl-64">
        <header className="admin-header sticky top-0 z-20 flex items-center justify-between border-b border-gold/20 bg-card/90 px-4 py-3 backdrop-blur lg:px-6">
          <button onClick={() => setOpen(true)} className="text-sage-deep lg:hidden"><Menu className="h-5 w-5" /></button>
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <ShieldCheck className="h-3.5 w-3.5 text-gold" />
            <span className="hidden sm:inline">Session securisee · </span>
        ...
        <main className="admin-main-shell flex-1 p-4 lg:p-6">{children}</main>

        <footer className="admin-footer mt-auto border-t border-gold/20 bg-card/60 px-4 py-3 text-center text-xs text-muted-foreground">
          <Heart className="inline h-3 w-3 text-gold" /> {footerLine}
        </footer>
      </div>
    </div>
  );

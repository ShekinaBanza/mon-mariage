...
export async function registerInvitation(input: RegistrationInput): Promise<RegistrationResult> {
  ...
  const requestedSeatIds = Array.from(new Set(input.requestedSeatIds ?? []));
  if (requestedSeatIds.length > 0 && requestedSeatIds.length !== requestedPeople) {
    throw new RegistrationError(
      "INVALID_SEATS",
      `Veuillez choisir ${requestedPeople} chaise${requestedPeople > 1 ? "s" : ""}.`
    );
  }
  ...
  return db.$transaction(async (tx) => {
  ...
  }).then(async ({ invitation, guest, members }) => {
    if (requestedSeatIds.length > 0) {
      try {
        ...
        try {
          ...
          // Best-effort cleanup only; the original seat error is returned below.
        }
        throw new RegistrationError(
          "SEATS_UNAVAILABLE",
          e?.message || "Les chaises choisies ne sont plus disponibles."
        );
      }
    } else if (settings.validationMode === VALIDATION_MODE.AUTO) {
      try {
    ...
    return {
      ...
      message:
        requestedSeatIds.length > 0 && finalInv!.assignmentState === ASSIGNMENT_STATE.ASSIGNED
          ? "Votre demande a ete enregistree avec la table et la chaise choisies."

...
import { deleteInvitationPermanently } from "@/lib/invitation-delete";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  switch (action) {
    ...
    case "delete": {
      if (!can(user.role, "guests:edit")) return NextResponse.json({ error: "Acces refuse." }, { status: 403 });
      const guestId = inv.guestId;
      await deleteInvitationPermanently(id);
      await db.activityLog.create({
        data: {
          ...
          entityId: id,
          ip,
          meta: JSON.stringify({ guestId }),
        },
      });
      return NextResponse.json({ success: true, message: "Invite supprime definitivement du systeme." });
    }
    default:

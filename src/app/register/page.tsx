...
import { User, Users, ArrowLeft, ArrowRight, Check, Sparkles, Heart, AlertCircle, Loader2, Armchair } from "lucide-react";
...
function normalizeCongoleseLocalNumber(value: string): string {
...
function zoneLabel(zone: string): string {
  if (zone === "groom") return "Cote homme";
  if (zone === "bride") return "Cote femme";
  return "Zone commune";
}
...
export default function RegisterPage() {
  ...
  const requiredSeats = type === "couple" ? 2 : 1;
  const selectedTable = tables.find((t) => t.id === selectedTableId);
  const invitationPath = result ? `/i/${result.publicToken}` : "";
  const invitationUrl = result ? `${origin || ""}${invitationPath}` : "";
  ...
  useEffect(() => {
    ...
    async function loadTables() {
      ...
      try {
        const res = await fetch(`/api/register?side=${form.side}&seats=${requiredSeats}`, { cache: "no-store" });
        const data = await res.json();
        if (!cancelled) {
          setTables(res.ok ? data.tables ?? [] : []);
        }
      } catch {
  ...
  function validate(): boolean {
    ...
    if (normalizeCongoleseLocalNumber(form.whatsapp).length !== 9) e.whatsapp = "Entrez les 9 chiffres apres +243";
    ...
    if (!selectedTableId) {
      e.placement = "Choisissez une table.";
    } else if (selectedSeatIds.length !== requiredSeats) {
      e.placement = `Choisissez ${requiredSeats} chaise${requiredSeats > 1 ? "s" : ""}.`;
    }
    setErrors(e);
    ...
    try {
      const res = await fetch("/api/register", {
        ...
        body: JSON.stringify({
          type,
          ...form,
          whatsapp: `${PHONE_PREFIX}${normalizeCongoleseLocalNumber(form.whatsapp)}`,
          requestedSeatIds: selectedSeatIds,
        }),
      });
      const data = await res.json();
      ...
      <div className="mx-auto w-full max-w-3xl flex-1 px-4 py-10">
        ...
        {step === "form" && (
          <div className="mt-8 space-y-6">
            ...
            <Card className="p-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Numéro WhatsApp *" error={errors.whatsapp}>
                  <div className="flex rounded-md border border-input bg-transparent shadow-xs focus-within:border-ring focus-within:ring-[3px] focus-within:ring-ring/50">
                    <span className="flex h-9 items-center border-r border-input px-3 text-sm font-medium text-sage-deep">
                      {PHONE_PREFIX}
                    </span>
                    <Input
                      value={form.whatsapp}
            ...
            <Card className="p-5">
              <div className="mb-4 flex items-center gap-2">
                <Armchair className="h-5 w-5 text-gold" />
                <h3 className="font-display text-lg text-sage-deep">Table et chaise</h3>
              </div>
              <div className="grid gap-4 sm:grid-cols-[1fr_1.2fr]">
                <Field label="Table *" error={!selectedTableId ? errors.placement : undefined}>
                  <Select
                    ...
                    disabled={placementLoading || tables.length === 0}
                  >
                    <SelectTrigger className="w-full">
                      <SelectValue placeholder={placementLoading ? "Chargement..." : "Choisir une table"} />
                    </SelectTrigger>
                    <SelectContent>
                      {tables.map((table) => (
                        <SelectItem key={table.id} value={table.id}>
                          Table {table.name} - {zoneLabel(table.zone)} ({table.availableCount} libre{table.availableCount > 1 ? "s" : ""})
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  {!placementLoading && tables.length === 0 && (
                    <p className="mt-2 text-xs text-destructive">Aucune table avec assez de chaises libres.</p>
                  )}
                </Field>

                <Field label={`Chaise${requiredSeats > 1 ? "s" : ""} *`} error={selectedTableId ? errors.placement : undefined}>
                  <div className="min-h-9 rounded-md border border-input p-2">
                    {selectedTable ? (
                            ...
                            <button
                              ...
                              className={`h-9 rounded-md border text-xs font-medium transition-colors ${
                                selected
                                  ...
                                  : "border-gold/30 bg-card hover:border-gold hover:bg-gold/10"
                              }`}
                            >
                              {seat.code}
                            </button>
                      ...
                      <p className="py-2 text-center text-xs text-muted-foreground">Choisissez d'abord une table.</p>
                    )}
                  </div>
                  {selectedTable && (
                    <p className="mt-2 text-xs text-muted-foreground">
                      {selectedSeatIds.length}/{requiredSeats} chaise{requiredSeats > 1 ? "s" : ""} selectionnee{requiredSeats > 1 ? "s" : ""}
                    </p>
                  )}
                </Field>
              </div>
